...
final class LogRepository {
	...
	public function upsert( string $url_path, ?string $query_hash, ?string $referer, ?string $user_agent, bool $allow_updates = true ): void {
		...
		$rows = $this->db->get_results(
			'SELECT id, hits FROM %i WHERE url_path = %s AND (query_hash = %s OR (query_hash IS NULL AND %s = \'\')) LIMIT 1',
			array( $table, $url_path, (string) $query_hash, (string) $query_hash ),
			\ARRAY_A
		);
		...
		if ( ! empty( $rows ) && isset( $rows[0]['id'] ) ) {
			...
			$id = (int) $rows[0]['id'];
			$this->db->query(
				'UPDATE %i SET hits = hits + 1, last_seen_at = %s, last_referer = %s, last_user_agent = %s WHERE id = %d',
				array( $table, $now, (string) $referer, (string) $user_agent, $id )
			);
			return;
		}

		$this->db->query(
			'INSERT INTO %i (url_path, query_hash, hits, first_seen_at, last_seen_at, last_referer, last_user_agent, status) VALUES (%s, %s, 1, %s, %s, %s, %s, \'open\')',
			array( $table, $url_path, (string) $query_hash, $now, $now, (string) $referer, (string) $user_agent )
		);
	}
	...
	public function list( int $page, int $per_page, string $status = '', ?string $search = null ): array {
		...
		$total        = (int) $this->db->get_var( $count_sql, $count_params );

		$list_sql    = 'SELECT id, url_path, query_hash, hits, first_seen_at, last_seen_at, last_referer, last_user_agent, status, matched_redirect_id FROM %i' . $where_sql . ' ORDER BY last_seen_at DESC LIMIT %d OFFSET %d';
		$list_params = array_merge( array( $table ), $params, array( $limit, $offset ) );
	...
	public function stats(): array {
		...
		$total_urls = (int) $this->db->get_var(
			'SELECT COUNT(*) FROM %i WHERE 1 = %d',
			array( $table, 1 )
		);
		$total_hits = (int) $this->db->get_var(
			'SELECT COALESCE(SUM(hits), 0) FROM %i WHERE 1 = %d',
			array( $table, 1 )
		);
		$open_urls  = (int) $this->db->get_var(
