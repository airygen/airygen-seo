<?php
/**
 * Aggregates queue status information for Broken Link Checker.
 *
...
/**
 * Builds queue metadata for the Broken Link Checker settings UI.
 */
final class StatusReporter {
	...
	private static function count_actions( string $status ): ?int {
		...
		$args = array(
			'hook'     => Constants::HOOK_BROKEN_LINK_CHECKER_RUN,
	...
	private static function next_run_gmt(): ?string {
		...
		$timestamp = as_next_scheduled_action(
			Constants::HOOK_BROKEN_LINK_CHECKER_RUN,
			array(),
			self::ACTION_GROUP
		);

		if ( false === $timestamp ) {
			$timestamp = as_next_scheduled_action( Constants::HOOK_BROKEN_LINK_CHECKER_RUN );
		}
