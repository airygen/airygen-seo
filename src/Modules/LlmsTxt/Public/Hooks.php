...
final class Hooks {
	...
	private static function maybe_output_llms_response(): void {
		...
		if ( Routes::is_llms_request() || self::is_path_request( '/llms.txt' ) ) {
			...
			echo self::render_base_content( $settings ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- text/plain llms.txt body.
			exit;
		}
		...
		if ( null !== $matched_extension_id ) {
			nocache_headers();
			header( 'Content-Type: text/plain; charset=UTF-8' );
			echo self::render_extension_content( $settings, $matched_extension_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- text/plain llms.txt extension body.
			exit;
		}
	}
