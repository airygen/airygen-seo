...
type SeoPressMigrationPanelProps = {
	restBase: string;
	isActive: boolean;
};

const DEFAULT_PROGRESS: SeoPressProgress = {
	total: 0,
	migrated: 0,
	remaining: 0,
	percent: 0,
...
const SeoPressMigrationPanel = ( {
	restBase,
	isActive,
}: SeoPressMigrationPanelProps ) => {
	const [ progress, setProgress ] = useState<SeoPressProgress>( DEFAULT_PROGRESS );
	const [ redirectsProgress, setRedirectsProgress ] =
		useState<SeoPressProgress>( DEFAULT_PROGRESS );
	...
	const statusPath = `${ normalizeBase( restBase ) }/migration/seopress`;
	const importPath = `${ normalizeBase( restBase ) }/migration/seopress/import`;
	const settingsPath = `${ normalizeBase( restBase ) }/migration/seopress/settings`;
	const redirectsPath = `${ normalizeBase( restBase ) }/migration/seopress/redirects`;

	const percent = useMemo( () => {
		const value = Number.isFinite( progress.percent ) ? progress.percent : 0;
		...
		<div className="space-y-6">
			<div className="space-y-2">
				...
				<div className="airygen_h1_description">
					{ __(
						'Prepare to migrate SEOPress metadata, social defaults, templates, and redirects into Airygen SEO.',
						'airygen-seo',
					) }
				</div>
			</div>
				...
				<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">
							{ __(
								'Import SEOPress per-post metadata (titles, descriptions, canonicals, social overrides, and robots).',
								'airygen-seo',
							) }
						</p>
					</div>
			...
			<section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
				<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">
							{ __(
								'Import SEOPress templates, social defaults, organization schema, and breadcrumbs.',
								'airygen-seo',
							) }
						</p>
					</div>
			...
			<section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
				<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">
							{ __(
								'Import post-level redirects from SEOPress and add them to Airygen Redirects.',
								'airygen-seo',
							) }
						</p>
					</div>
				...
				<div className="airygen-setting-card__input--normal rounded-lg border border-slate-200 p-4">
					<div className="space-y-2">
						<div className="flex items-center justify-between text-xs text-slate-500">
							<span>{ __( 'Progress', 'airygen-seo' ) }</span>
							<span>{ `${ String( redirectPercent ) }%` }</span>
						</div>
						<div className="h-2 w-full overflow-hidden rounded-full bg-slate-100">
							<div
								className="h-full rounded-full bg-sky-500 transition-all"
...
export default SeoPressMigrationPanel;
