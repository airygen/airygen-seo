...
type YoastMigrationPanelProps = {
	restBase: string;
	isActive: boolean;
};

const DEFAULT_PROGRESS: YoastProgress = {
	total: 0,
	migrated: 0,
	remaining: 0,
	percent: 0,
...
const YoastMigrationPanel = ( { restBase, isActive }: YoastMigrationPanelProps ) => {
	const [ progress, setProgress ] = useState<YoastProgress>( DEFAULT_PROGRESS );
	const [ redirectsProgress, setRedirectsProgress ] =
		useState<YoastProgress>( DEFAULT_PROGRESS );
	...
	const statusPath = `${ normalizeBase( restBase ) }/migration/yoast`;
	const importPath = `${ normalizeBase( restBase ) }/migration/yoast/import`;
	const settingsPath = `${ normalizeBase( restBase ) }/migration/yoast/settings`;
	const redirectsPath = `${ normalizeBase( restBase ) }/migration/yoast/redirects`;
	...
	const redirectsLabel = useMemo(
		() =>
			`${ redirectPercent }% — ${ redirectsProgress.migrated } ${ __( 'posts imported', 'airygen-seo' ) } / ${ redirectsProgress.total } ${ __( 'total', 'airygen-seo' ) }`,
	...
	const startImport = useCallback( async () => {
		...
		try {
			...
			while (
				...
				// Small delay keeps the UI responsive while batching requests.
		...
		<div className="space-y-6">
			<div className="space-y-2">
				...
				<div className="airygen_h1_description">
					{ __(
						'Prepare to migrate Yoast SEO metadata, social defaults, and schema settings into Airygen SEO.',
						'airygen-seo',
					) }
				</div>
			</div>
				...
				<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">
							{ __(
								'Import per-post SEO metadata (titles, descriptions, social overrides, and schema).',
								'airygen-seo',
							) }
						</p>
					</div>
			...
			<section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
				<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">
							{ __(
								'Import global defaults such as title templates, schema organization, and social defaults.',
								'airygen-seo',
							) }
						</p>
					</div>
			...
			<section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
				<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">
							{ __(
								'Import redirects saved per post in Yoast and add them to Airygen Redirects.',
								'airygen-seo',
							) }
						</p>
					</div>
...
export default YoastMigrationPanel;
